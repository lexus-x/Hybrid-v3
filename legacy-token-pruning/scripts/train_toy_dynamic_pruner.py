...
def build_argparser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Train a class-project toy DynamicViT model.")
    parser.add_argument("--model", choices=["dense", "pruned"], default="pruned")
    parser.add_argument("--data-dir", type=Path, default=ROOT / "data")
    parser.add_argument("--output-dir", type=Path, default=ROOT / "outputs" / "train_run")
    ...
    parser.add_argument("--keep-ratios", type=str, default="0.5,0.5")
...
def main() -> None:
    ...
    if args.pretrained_checkpoint is not None:
        print(f"Loading pretrained backbone from {args.pretrained_checkpoint}")
        checkpoint = torch.load(args.pretrained_checkpoint, map_location=device, weights_only=True)
        # Load state dict with strict=False because the classification head is randomly initialized and 
        # may not be in the pretrained checkpoint (which only has the backbone features).
        # We also might have extra pruning modules if args.model == "pruned".
    ...
    if args.teacher_checkpoint is not None:
        teacher_model = load_checkpoint_model(args.teacher_checkpoint, device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if "avg_kept_tokens_per_stage" in eval_metrics:
            row["avg_kept_tokens_per_stage"] = eval_metrics["avg_kept_tokens_per_stage"]
        history.append(row)

        print(
            f"epoch={epoch} "
            f"train_loss={row['train_loss']:.4f} ce={row['train_ce_loss']:.4f} "
            f"kd={row['train_kd_loss']:.4f} train_acc={row['train_accuracy']:.4f} "
